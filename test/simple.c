...
int main(int argc, char** argv)
{
  ...
  gasAnimation* animation1 = gasNumberAnimationFromToNew(GAS_NUMBER_ANIMATION_TARGET_X, GAS_EASING_LINEAR, 0.0f, 800.0f, 5.0f);

  glhckObject* cube2 = glhckCubeNew(10);
  glhckObjectPositionf(cube2, WIDTH/2, 0, 0);
  gasAnimation* animation2 = gasNumberAnimationFromToNew(GAS_NUMBER_ANIMATION_TARGET_Y, GAS_EASING_QUAD_OUT, 0.0f, 480.0f, 4.0f);

  glhckObject* cube3 = glhckCubeNew(10);
  glhckObjectPositionf(cube3, WIDTH/4, HEIGHT/4, 0);
  gasAnimation* animation3 = gasNumberAnimationFromToNew(GAS_NUMBER_ANIMATION_TARGET_ROT_Z, GAS_EASING_QUAD_IN, 0.0f, 720.0f, 5.0f);
